...
function NSXConfigPage() {
  ...
  const loadConfigs = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/nsx-configs');
      setConfigs(response.data);
    } catch (error) {
      console.error('Error loading NSX Manager configurations:', error);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await axios.put(`/api/nsx-configs/${editingId}`, formData);
        setMessage({ type: 'success', text: 'NSX Manager configuration updated successfully' });
      } else {
        await axios.post('/api/nsx-configs', formData);
        setMessage({ type: 'success', text: 'NSX Manager configuration created successfully' });
      }
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      await axios.delete(`/api/nsx-configs/${id}`);
  ...
  const handleTest = async (id, name) => {
    setTestingId(id);
    try {
      const response = await axios.post(`/api/nsx-configs/${id}/test`);
  ...
  if (loading) {
    return (
      <div className="loading">
        ...
        <p>Loading NSX Manager configurations...</p>
      </div>
    );
  }

  return (
    <div className="nsx-config-page">
      <div className="page-header">
        <h1 className="page-title">NSX Manager Configuration</h1>
        {!showForm && (
          <button 
        ...
        <div className="config-form-container">
          <div className="config-form">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2>{editingId ? 'Edit NSX Manager Configuration' : 'Add New NSX Manager'}</h2>
              <button 
            ...
            <form onSubmit={handleSubmit}>
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder="e.g., Production-NSX"
                  required
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder="e.g., nsx-manager.company.com"
                  required
                  className="form-input"
                />
                <small>FQDN or IP address of NSX Manager</small>
              </div>
              ...
              <div className="form-group">
                <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                  ...
                  />
                  Enabled (collect data from this NSX Manager)
                </label>
              </div>
      ...
      <div style={{ marginTop: '2rem' }}>
        <h2 style={{ marginBottom: '1rem', color: '#2c3e50' }}>
          Configured NSX Managers ({configs.length})
        </h2>

        {configs.length === 0 ? (
          <div className="empty-state">
            <h3>No NSX Manager Configurations</h3>
            <p>Click "Add NSX Manager" to create your first configuration.</p>
          </div>
        ) : (
          <div className="version-table">
            ...
            {configs.map((config) => (
              <div key={config.id} className="table-row" style={{ gridTemplateColumns: '2fr 2fr 2fr 1fr 2fr' }}>
                <div className="component-name">{config.manager_name}</div>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                  <button
                    onClick={() => handleTest(config.id, config.manager_name)}
                  ...
                  <button
                    onClick={() => handleDelete(config.id, config.manager_name)}
      ...
      <div style={{ 
        ...
        <ul style={{ marginTop: '1rem', lineHeight: '1.8', marginLeft: '1.5rem' }}>
          <li>Use descriptive names to easily identify your NSX Managers (e.g., "Prod-NSX", "DR-NSX")</li>
